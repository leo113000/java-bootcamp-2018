...
@RunWith(SpringRunner.class) @SpringBootTest public class UserServiceTest {
	...
	@Test public void whenCreateAnUserThenTheUserExistsInTheRepository() {
		...
		this.userService.register(aMock.getEmail(), aMock.getUsername(),aMock.getPassword());
	...
	@Test public void whenGetByIdThenReturnTheRightUser() {
		...
		this.userService.register(anotherMock.getEmail(),  anotherMock.getUsername(),anotherMock.getPassword());
		when(target.getId()).thenReturn(id);
		when(target.getUsername()).thenReturn("targetMocky");
		this.userService.register(target.getEmail(), target.getUsername(),target.getPassword());
	...
	@Test public void whenGetByUsernameThenReturnTheRightUser() {
		...
		this.userService.register(u.getEmail(), u.getUsername(),u.getPassword());
	...
	@Test public void whenUpdateAnUserThenModifyTheUserInTheRepository() {
		...
		this.userService.register(anotherMock.getEmail(), anotherMock.getUsername(),anotherMock.getPassword());
		this.userService.register(original.getEmail(), original.getUsername(),original.getPassword());
		assertEquals("original user", this.userService.getById(id).getUsername());
		this.userService.updateById(id, modified.getEmail(),modified.getUsername(),modified.getPassword());
