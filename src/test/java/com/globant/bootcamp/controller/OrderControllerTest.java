...
@RunWith(SpringRunner.class) @SpringBootTest public class OrderControllerTest {
	...
	@Test public void testGetEndpoint() throws Exception {
		this.mockMvc.perform(get("/orders").header("Authorization", token).accept("application/json;charset=UTF-8")).andDo(print())
	...
	@Test public void testCreateOrder() throws Exception {
		...
		this.mockMvc.perform(post("/orders").header("Authorization", this.token).param("paymentMethodId", "1").param("deliverMethodId", "1")
	...
	@Test public void testWrongParametersCreateOrder() throws Exception {
		...
		this.mockMvc.perform(
				post("/orders").header("Authorization", this.token).param("paymentMethodId", "155").param("deliverMethodId", "15")
						.accept("application/json;charset=UTF-8")).andDo(print()).andExpect(status().is4xxClientError());
	}
